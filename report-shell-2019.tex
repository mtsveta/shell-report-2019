...
\eqref{subsec:part-2} is dedicated to describe the performance of smart approach on the chemical system 
with kinetically controlled species. Finally, Subsection \eqref{subsec:part-3} highlights results of 
...
$n=(n_{1},\ldots,n_{\mathrm{N}})\in\mathbb{R}^{\text{N}}$ is the vector of species amounts 
(speciation), with $n_{i}$ denoting the amount of the $i$th species (in moles) and N the number of 
species, and $b=(b_{1},\ldots,b_{\mathrm{E}})\in\mathbb{R}^{\text{E}}$ is the vector of element 
amounts with $b_{j}$ being the amount of the $j$th element (in moles) and $\text{E}$ the number of 
...
speciation to equilibrium and the kinetic species with corresponding amounts $n_{e}$ and $n_{k}$ and 
bulk compositions $b_{e}$ and $b_{k}$. Then, the total amount of chemical species is presented as 
\[
n=\begin{bmatrix}n_{e}\\n_{k}\end{bmatrix}:\mathbb{R}^{N}\rightarrow\mathbb{R}^{N}
\]
...
Let $\mathcal{I}_e$/$\mathcal{I}_k$ represent the sets of input conditions for the chemical 
equilibrium/kinetics problems that have been fully solved (i.e., using the chemical equilibrium 
function $\varphi$ or system of ODEs \eqref{eq:kinetics}):
%
\begin{equation}
\mathcal{I}_e =\left\{ (T^{i},P^{i},b_e^{i})\right\}_{i=1}^{\text{I}} 
\qquad \mbox{or} \qquad 
\mathcal{I}_k =\left\{ (n^j_k)\right\}_{j=1}^{\text{J}}.
\end{equation}
%
Furthermore, let $d^{i}$ denote the Euclidean distance from $(T^{i},P^{i},b^{i})$, the input 
condition of the $i$th fully solved problem, to $(T,P,b)$, the given input condition for the new 
equilibrium/kinetics calculation:
%
\begin{equation}
d^{i}=\left[\left(T-T^{i}\right)^{2} 
+\left(P-P^{i}\right)^{2}
+\sum_{j=1}^{\text{E}}\left(b_{j}-b_{j}^{i}\right)^{2}\right]^{\frac{1}{2}} 
\qquad i=1,\ldots,\mathrm{I}.
\end{equation}

%---------------------------------------------------------------------------------------------------%
For the first-order Taylor expantion presented in (Eq.~\ref{eq:smart-estimate} or 
Eq.~\ref{eq:smart-estimate-kinetics}), a potentially suitable candidate for 
$(T^{\star},P^{\star},b_e^{\star})$ or $(n^\circ_k)^\star$ is the \emph{nearest neighbor (NN)} of 
$(T,P,b)$ or $n^\circ_k$ among all inputs in $\mathcal{I}_e$ or $\mathcal{I}_k$ (i.e., saved input 
with the shortest Euclidean distance to $(T,P,b)$ or $n^\circ_k$). Note that other norms and 
strategies can be used to define the most optimal reference element for the prediction.
...
\begin{figure}
\begin{centering}
\includegraphics{figures/equilibrium/algorithm-flowchart-for-equilibrium}
\par\end{centering}
...
acceptance test would be less complete than Eq.~(\ref{eq:acceptance-test-mu})
...
For kinetic species, we test the variation in the kinetic rates, i.e., 
%
\begin{equation}
|d r_i|  \leq \epsilon_{{\rm  rel}} |\ln r_{i}^{\star}| + 
                   \epsilon_{{\rm  abs}} \qquad (i=1,\ldots,{\rm N_k}),
\label{eq:acceptance-test-rates}
\end{equation}
%
where $\delta r_i = \tfrac{d r^i}{d n_k} \delta n$.
...
\subsection{Pitzer activity model. Local equilibrium assumption.  \label{subsec:part-1}}
