...
\sveta{
We also point out that after curtain amount of the time steps, trainings that are expected to be 
...
by 1~\%, whereas, the inaccuracy of the aqueous concentrations reproduced by the on-demand learning 
is in average ?? \& but might get up to 10 or 16~\% in several cells throughout simulation. 
TODO: check the average value!
}
\begin{figure}[!t]
\begin{centering}
\subfloat[]{\includegraphics[width=0.6\textwidth]{figures/equilibrium/on-demand-learning-countings}
\label{fig:number-training-1}} \\
\subfloat[]{\includegraphics[width=0.6\textwidth]{figures/equilibrium/on-demand-learning-total}
...
and (b) accumulated during the reactive transport simulation. In Figure \ref{fig:number-training-2}, 
...
\begin{algorithm}[H]
\SetAlgoLined
\KwResult{species amounts $n$ on the step $[t_k, t_{k+1}], k = 1, \ldots, {\rm K}$}
...
\caption{The ODML algorithm for chemical kinetics for each reactive transport step.}
