...
% I think we do not need this, because we have the Summary below
% \begin{abstract}
% This report summarizes the results obtained during the first year of the project ``Reaktoro as 
...
next year. 
\begin{center}
\textbf{\emph{\small{}The summary below aims to be self-contained in communicating the latest achievements of this project in a concise way. However, for a deeper understanding of some parts, further reading of the subsequent sections might be required.}}{\small\par}
\par\end{center}
...
(Davies, Debye-H\"uckel), but also because it is one of most computationally
...
\textbf{Goal 1}: Accelerate geochemical reaction calculations (chemical equilibrium 
and chemical kinetics) in reactive transport simulations using a novel on-demand machine learning algorithm.

\textbf{Goal 2}: Implement missing geochemical capabilities and models in Reaktoro such as redox reactions, exchange, surface complexation, solid solutions, and other modeling capabilities.

\textbf{Goal 3}: Implement native support for PHREEQC databases in Reaktoro.

For the past year, our focus has primarily been on advancing \textbf{Goal 1}, 
the most challenging deliverable of this project. 

We summarize below the results so far achieved followed by the major tasks 
we will concentrate next year. 

\subsubsection*{Our results this year}
%---------------------------------------------------------------------------------------------------%
\begin{enumerate}\setlength{\itemsep}{0pt} \setlength{\parskip}{0pt}
\item The on-demand machine learning algorithm (ODML) was extensively tested in a 1D reactive 
transport problem, where chemistry is controlled by local equilibrium and a Pitzer activity model 
is used for the aqueous solution. The chemical system consisted of 36~species in 4~phases 
(an aqueous phase and three mineral phases, namely, calcite, dolomite and quartz). 
The following results have been obtained:
%
%---------------------------------------------------------------------------------------------------%
\begin{itemize}
\setlength{\itemsep}{0pt} \setlength{\parskip}{0pt}
\item The ODML algorithm yielded a speedup of {\bf 200} (in the worst case) and 
{\bf 400} (in the best\slash zero-cost-search case) for the chemical equilibrium calculations
in the reactive transport simulations.
%
\item Only {\bf 258} chemical equilibrium states out of {\bf 1,000,000} were fully evaluated
(by triggering a Gibbs energy minimization (GEM) computation); the rest, {\bf 99.97} \%, were 
quickly and accurately estimated using exact sensitivity derivatives.
%
...
for the concentrations of the aqueous species.
%
\item Main results, obtained by testing the algorithm on the above-described benchmark, are 
summarized in the paper recently submitted to journal Transport in Porous Media.
\end{itemize}
%
\item The ODML algorithm was extended for reactive transport simulations with chemical kinetics 
for mineral dissolution\slash precipitation reactions.  Our preliminary results, using a similar
reactive transport problem, with calcite dissolving kinetically, produced the following results:
%
\begin{itemize} 
\setlength{\itemsep}{0pt} \setlength{\parskip}{0pt}
\item A speedup of {\bf 8-10x} in the chemical kinetics computations, compared to the conventional
algorithm;
%
\item A speedup of {\bf 30x} and {\bf 100x} (in the worst and best case, respectively) when 
combining accelerated chemical kinetics of mineral dissolution with accelerated chemical 
equilibrium calculations between the aqueous species.
%
\end{itemize}
%---------------------------------------------------------------------------------------------------%
%
...
{\bf 92-124x} for tree kinetic minerals.
%
...
%---------------------------------------------------------------------------------------------------%
\subsubsection*{Our planned tasks next year}
%
\begin{enumerate} \setlength{\itemsep}{0pt} \setlength{\parskip}{0pt}


\item Improve the acceptance criteria in the ODML algorithm so that fewer training operations
are triggered when more complex chemical setups are considered.
%
\item Improve the search operations in the algorithm (needed to identify a previously 
trained problem that is potentially similar to the new one). We plan to 
%---------------------------------------------------------------------------------------------------%
\begin{itemize} 
\setlength{\itemsep}{0pt} \setlength{\parskip}{0pt}
\item combine tree-based (kd-tree) and linear search algorithms,
\item experiment with locally sensitive hashing tables (LSH),
\item apply a ranking system to the learned problems, which will permit us to search first 
those trained problems that are mostly used for predictions

% Let's not mention this one, because this could seem as a promise; and it's already a lot to do.
%\item experiment cluster using reference points clustering/ranking.
\end{itemize}
%---------------------------------------------------------------------------------------------------%
%
\item Accelerate the aqueous\slash gaseous chemical equilibrium calculations needed during the evaluation of
the reaction rates of minerals (e.g., when computing the current saturation indices of the minerals), 
which can be very computationally expensive.
%
% This won't work well I guess. There are too many inputs (high-dimension input space T, P, n) 
...
%
\item Test the ODML algorithm on more complex scenarios:
%---------------------------------------------------------------------------------------------------%
\begin{itemize} 
\setlength{\itemsep}{0pt} \setlength{\parskip}{0pt}
\item reactive transport resulting from the attack of seawater into cement and concrete systems, 
considering for this about \~100 aqueous species, \~60 pure minerals, and \~10 solid solutions),
\item chemical systems with more than one kinetically controlled mineral species,
\item reactive transport in 2D (and possibly 3D) heterogeneous porous media using Rok, a simulator resulting 
from the combination of Reaktoro and Firedrake.
\end{itemize}
%---------------------------------------------------------------------------------------------------%
%  This won't be appreciated (best not to mention)
...
%
\sveta{
\item Implement improved support for modeling redox reactions in Reaktoro.
%
...
$b=(b_{1},\ldots,b_{\mathrm{E}})\in\mathbb{R}^{{\rm E}}$ is the vector of element 
