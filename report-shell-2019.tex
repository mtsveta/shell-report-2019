...
equation~(\ref{eq:smart-estimate}).
%---------------------------------------------------------------------------------------------------%
...
%---------------------------------------------------------------------------------------------------%
The smart algorithm for kinetically controlled reactions is presented with the flowchart in Figure~\ref{fig:algorithm-diagram-kinetics}. It begins with integrating the system of ODEs 
...
a reference to estimate the new kinetic state using equation~(\ref{eq:smart-estimate-kinetics}).
...
rock column consists of 98\%$_{{\rm vol}}$ SiO$_{2}$(quartz) and 2\%$_{{\rm vol}}$
...
\emph{Solid circles} ${\rm (}\bullet{\rm )}$ indicate those cells, where successful
fast and accurate smart equilibrium predictions were preformed successfully. The \emph{empty circles} ${\rm (}\circ{\rm )}$ depict the cells,
...
\subsection{HKF activity model. Example with chemical kinetics.  \label{subsec:part-2}}
{\color{blue}{UPDATE:
