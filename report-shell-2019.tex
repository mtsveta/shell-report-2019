...
\mathcal{I}_k =\left\{ (n^{\circ}_k)^j \right\}_{j=1}^{\text{J}}.
\end{equation}
%
...
\left(T-T^{i}\right)^{2} 
+\left(P-P^{i}\right)^{2}
+\sum_{l=1}^{\text{E}}\left(b_{l}-b_{l}^{i}\right)^{2}\right]^{\frac{1}{2}} 
\qquad i=1,\ldots,\mathrm{I} 
\qquad \mbox{and} \qquad 
d^{j}_k = \left[
\sum_{m=1}^{\text{N_k}}\left((n^{\circ}_{k, m})^j - n^\circ_{k, m}\right)^{2}\right]^{\frac{1}{2}} 
\end{equation}
%
...
whereas $d^{j}_k$ measures the disctance between the initial condition of $j$th fully integrated 
